...
//! Two layers:
//!
//! - **Pure, ort-free** (compile with `--no-default-features`): the BILUO
//!   decode, the label-map loading, and the hermetic worker tests. The
//!   config schema, the `OrtError` type, the `OrtSessionRegistry` with its
//!   `SessionLoader` DIP seam, and the neutral consumer types live in
//!   `fluent-llm` (`onnx_config`, `onnx_error`, `onnx_session`, `backend`)
//!   and are re-exported here, so hermetic tests exercise all of this
//!   without a model.
...
pub use pii::{decode_biluo, load_id2label};
