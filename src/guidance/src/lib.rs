...
//! | `enhancer` | LLM comment generation | `guidance-llm`, `fluent-types` |
...
//! | `query_engine` | Explain/query orchestration | `fluent-types`, `search-vector`, `guidance-project-knowledge`, `query/*` |
//! | `query` sub-modules | Query pipeline: identifier, strategy, filters, synthesizer, snapshot | `fluent-types`, `regex` |
//! | `sync` sub-modules | JSON store, file lock, staleness, comment management | `fluent-types`, `fs2`, `common-core` |
//!
