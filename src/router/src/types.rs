...
impl RouterResponse {
    ...
    pub fn error(model: impl Into<String>, err: impl AsRef<str>) -> Self {
        Self::text(model, format!("ERROR: {}", err.as_ref()))
    }

    pub fn fallback(model: impl Into<String>) -> Self {
        Self::text(
            model,
            "ERROR: route ends in unsupported deterministic response.",
        )
    }

    pub fn answer_text(&self) -> Option<String> {
        self.choices.first().map(|c| c.message.content.to_string_lossy())
    }
}
