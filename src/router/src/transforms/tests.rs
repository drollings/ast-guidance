...
mod tests {
    ...
    #[test]
    fn test_secret_mask_bearer_token() {
        let transform = crate::transforms::secret_mask::SecretMask;
        let request = make_request("Authorization: Bearer abc123xyz");
        let result = transform.transform(&request, &[]).unwrap();
        let output = result.messages[0].content.to_string_lossy();
        assert!(output.contains("Bearer ****"), "got: {output}");
        assert!(!output.contains("abc123xyz"), "got: {output}");
    }

    #[test]
    fn test_secret_mask_password_keyvalue() {
        let transform = crate::transforms::secret_mask::SecretMask;
        let request = make_request("password=superSecret123!");
        let result = transform.transform(&request, &[]).unwrap();
        let output = result.messages[0].content.to_string_lossy();
        assert!(output.contains("password=****"), "got: {output}");
        assert!(!output.contains("superSecret"), "got: {output}");
    }

    #[test]
    fn test_secret_mask_no_op_on_clean_text() {
        let transform = crate::transforms::secret_mask::SecretMask;
    ...
    #[test]
    fn test_secret_mask_name() {
        let transform = crate::transforms::secret_mask::SecretMask;
        assert_eq!(transform.name(), "secret_mask");
    }

    // ── M3.3: Sanitize ─────────────────────────────────────────────────

    #[test]
    fn test_sanitize_bidi_removed() {
        let transform = crate::transforms::sanitize::Sanitize;
        let request = make_request("hello\u{202E}world");
        let result = transform.transform(&request, &[]).unwrap();
        assert_eq!(result.messages[0].content.to_string_lossy(), "helloworld");
    }

    #[test]
    fn test_sanitize_ansi_stripped() {
        let transform = crate::transforms::sanitize::Sanitize;
        let request = make_request("\u{1B}[31mRED\u{1B}[0m normal");
        let result = transform.transform(&request, &[]).unwrap();
        assert_eq!(
            result.messages[0].content.to_string_lossy(),
            "RED normal"
    ...
    #[test]
    fn test_sanitize_normal_text_preserved() {
        let transform = crate::transforms::sanitize::Sanitize;
        let request = make_request("Hello, 世界! 😀");
        let result = transform.transform(&request, &[]).unwrap();
        assert_eq!(
            result.messages[0].content.to_string_lossy(),
            "Hello, 世界! 😀"
    ...
    fn test_sanitize_name() {
        let transform = crate::transforms::sanitize::Sanitize;
        assert_eq!(transform.name(), "sanitize");
    }
