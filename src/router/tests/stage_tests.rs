...
mod tests {
    ...
    /// Post-M3c flat-engine harness: the flat `ClassifierStage` is assembled
    /// directly over an explicit `RoutingConfig` — the same stages the
    /// builder's flat branch wires (prefilter + classifier + optional retry
    /// decorator + target-matching ladder), minus the deleted flat authoring
    /// surface. `routes` are `(route, group)` pairs; models/groups come from
    /// the fixture config.
    fn flat_engine_pipeline(
        ...
        let matcher =
            if params.target_match == crate::config::builder::TargetMatchMode::SelfAssess {
        ...
        let classifier: Arc<ClassifierStage> = Arc::new(ClassifierStage::new(
            client,
            routing,
            ...
            1,
            params.classifier_model.clone().unwrap_or_else(|| "fast".into()),
            limiter,
            matcher,
        ...
        orchestrator = orchestrator.push(Arc::new(DeterministicPreFilter::new()));
        if params.classifier_retry_max > 0 {
            let retry: Arc<crate::stages::retry_classifier::RetryClassifier> =
                Arc::new(crate::stages::retry_classifier::RetryClassifier::new(
                    classifier,
                    params.classifier_retry_max as usize,
                    params.classifier_retry_prompts.clone(),
                ));
    ...
    fn run_matrix_pipeline(
        ...
        let pipeline = flat_engine_pipeline(config, &[("code", "code"), ("local", "local")], backend);
        let mut ctx = WorkContext::default();
        ctx.set_structured(
            "request",
            &serde_json::json!({
    ...
    fn retry_classifier_recovers_through_real_pipeline() {
        ...
        let (result, logs) = capture_logs(|| {
            let pipeline = flat_engine_pipeline(
                &config,
                &[("code", "code")],
    ...
    fn retry_exhausts_then_default_policy_rejects() {
        ...
        let pipeline = flat_engine_pipeline(
            &config,
            &[("code", "code")],
            std::sync::Arc::new(AlwaysGarbageBackend),
        );
        let mut ctx = WorkContext::default();
        ctx.set_structured(
            "request",
    ...
    fn run_ladder_pipeline(
        ...
        let pipeline = flat_engine_pipeline(config, &[("code", "code")], backend);
        let mut ctx = WorkContext::default();
        ctx.set_structured(
            "request",
            &serde_json::json!({
    ...
    fn always_route_forces_dispatch_over_classifier_respond() {
        ...
        let (decision, target) = stage
            .evaluate_with_target(&ctx, &[])
            .expect("evaluate");
        ...
        let rt = target.expect("must dispatch");
        ...
        let (decision_local, target_local) = stage
            .evaluate_with_target(&ctx_local, &[])
            .expect("evaluate");
        ...
        assert!(
            target_local.is_none(),
            "non-always-route keeps the direct classifier answer"
        );
    }

    #[test]
    fn nlp_stage_is_wired_through_the_pipeline_builder() {
        // `nlp: true` with no classifier model resolved → the stage runs the
        // deterministic rule pipeline (no LLM fetch), publishes the signals.
        let config: crate::config::RouterConfig = serde_json::from_str(
            r#"{
        ...
        .expect("valid nlp config");

        let pipeline = config
            .build_named_pipeline_with_backend("default", None)
            .expect("nlp pipeline should build");
        ...
        let nlp = result
            ...
            .expect("an Nlp decision is present");
        let signals = StageMetadata::from(nlp.metadata.clone())
            .nlp_parse()
