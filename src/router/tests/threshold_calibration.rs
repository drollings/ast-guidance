...
// ── Dual-harness mirror ──────────────────────────────────────────────
// This file is mirrored at `src/tests/threshold_calibration.rs`, compiled
// into the lib unit tests. The two copies share identical test bodies; only
// this header differs (here `fluent_router` is the extern lib under test, so
// no alias is needed — the unit copy declares `use crate as fluent_router;`).
...
fn m5a_hnsw_recall_at_scale_and_brute_force_control() {
    ...
    for n in [256usize, 512, 1024, 2048] {
        ...
        let mut per_query = Vec::with_capacity(20);
        for q in 0..20 {
            ...
            let approx: HashSet<i64> =
                fluent_db::hnsw::hnsw_lookup(&idx, &query, k).expect("built → Some")
                    ...
                    .map(|(id, _)| id)
                    .collect();
            per_query.push(approx.intersection(&truth).count() as f64 / k as f64);
        }
        ...
        if n >= 512 {
            assert!(
                ...
                "M5a [B]: HNSW recall@{k} vs brute force at N={n} must be ≥0.95, got {mean_recall:.3} \
                 (index len={} for {n} inserts; per-query recalls: {per_query:.2?})",
                idx.len(),
            );
        }
    }
...
fn m5b_ef_sweep_holds_recall_and_k0_probes_nothing() {
    ...
    // recall ≥ 0.90; the `.max(64)` floor in `HnswIndex::search` is what
    // production actually uses at small k (production's ef=64 has the
    // separate 0.95 gate in m5a).
    // Measured 2026-09-04 at N=1024 (6 runs + a 40-run tail sample):
    // ef=20 → 0.940–1.000, ef=40 → 0.930–1.000, ef=80 → 0.970–1.000.
    // Halving ef below k*4 costs recall — the floor is load-bearing — and
    // the OS-seeded level draw has a genuine ~0.93 tail at ef=40, so the
    // gate sits at 0.90 (margin below the observed tail, not at it).
    use std::collections::HashSet;
    let dim = 32;
    let n = 1024;
    let k = 10;
    ...
    for ef in [k * 2, k * 4, k * 8] {
        ...
        if ef == k * 4 {
            assert!(
                mean_recall >= 0.90,
                "M5b [B]: ef=k*4 must hold recall ≥0.90, got {mean_recall:.3}"
            );
        }
    }
